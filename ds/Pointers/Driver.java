...
public class Driver{
	...
	while (walker != null){
	//Note that its "walker" itself that is null when there's nothing it is pointing to.  walker.getData requires there to be something pointed at.	
		System.out.println(walker.getData());
		//System.out.println(walker);//This code prints out the contents and puts and arrow next to each!!!  That's because of the toString method in Node.java
		walker = walker.getNext();
