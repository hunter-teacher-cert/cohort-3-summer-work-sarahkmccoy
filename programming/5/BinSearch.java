 ...
 * Sarah McCoy
 * collaborators: Rachel Kaufman, Stephen Sabaugh
 */
...
public class BinSearch
{
  ...
  */
  ...
  {
    //Q: Why did the designers of this class opt for 2 binSearch methods instead of 1?  A:---because its easier to run from the main-otherwise you would need to know the length of the array, and type in the low and hi indices
  ...
  public static int binSearchRec( int[] a, int target,
                                  int lo, int hi )//changed this to lo/hi because of following code, and easier to type
  {
    int tPos = -1; //init return var to flag/signal value

    int mPos = (lo + hi) / 2; //init tracker var for middle position.  Int will automatically truncate any half value, so realize that its "rounding down"
